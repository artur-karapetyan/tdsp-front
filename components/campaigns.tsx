...
export default function Campaigns(props: Props) {
  ...
  const handleCheckboxChange = async (id: number, is_enabled: boolean) => {
    ...
    queryClient.invalidateQueries(["campaign", page]);
  };
        ...
        {data.map((item) => (
          ...
          >
            <div className="flex justify-start items-start self-stretch flex-grow-0 flex-shrink-0 w-[170px] relative overflow-hidden gap-2.5 px-[19px]">
              <input
                type={"checkbox"}
                className="flex-grow-0 flex-shrink-0 w-[30px] h-7 relative overflow-hidden bg-[#3193f5]"
                onChange={() => handleCheckboxChange(item.id, item.is_enabled)}
                checked={item.is_enabled}
              />
            </div>
            <p className="self-stretch flex-grow-0 flex-shrink-0 w-[100px] h-7 text-base font-medium text-left text-black">
              {item.id}
            </p>
            <p className="self-stretch flex-grow-0 flex-shrink-0 w-[240px] h-7 text-base font-medium text-left text-black">
              {item.name}
            </p>
            <p className="self-stretch flex-grow-0 flex-shrink-0 w-[170px] h-7 text-base font-medium text-left text-black">
              {item.budget}
            </p>
            <div className="flex justify-start items-start self-stretch flex-grow-0 flex-shrink-0 w-[170px] relative overflow-hidden gap-2.5 px-[19px]">
              <input
                type={"number"}
                className="flex-grow-0 flex-shrink-0 w-20 h-7 text-center relative overflow-hidden bg-[#e6e5e5]"
                defaultValue={item.min_bid}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                  e.preventDefault();
                  handleBidChange(item.id, e);
                }}
              />
            </div>
          </div>
