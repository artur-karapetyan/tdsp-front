...
function fetcherGenerator(page: number) {
  return async function (): Promise<FetcherResult> {
    ...
    const response = await axios.get<FetcherResult>(
      `http://${process.env.NEXT_PUBLIC_HOST}/api/notify/${page}/`,
      {
        headers: {
    ...
    return (
      <>
        {data.map((item) => (
          <div
            key={item.id}
            className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 h-9 relative overflow-hidden gap-2 px-[5px] py-1 bg-neutral-100 hover:bg-[#85b0ed] duration-200"
          >
            <p className="self-stretch flex-grow-0 flex-shrink-0 w-[120px] h-7 text-base font-medium text-left text-black">
              {item.id}
            </p>
            <p className="self-stretch flex-grow-0 flex-shrink-0 w-[120px] h-7 text-base font-medium text-left text-black">
              {item.notify_id}
